...
class CheckoutController extends Controller
{
    ...
    /**
     * Step 6: Process payment (simulation or real provider integration)
     */
    public function processPayment(Request $request)
    {
        ...
        if ($request->payment_status !== 'success') {
            return back()->withErrors(['payment' => 'Payment failed or cancelled.']);
        }

        $itemIds = session('checkout_items', []);
        if (empty($itemIds)) {
            return redirect()->route('home')->withErrors(['checkout' => 'No items found in checkout.']);
        }
        ...
        $endDate = Carbon::now()->addMonths((int)$duration);

        DB::transaction(function () use ($customer, $items, $duration, $startDate, $endDate) {
            foreach ($items as $item) {
                Subscription::create([
                        'checkout_customer_id' => $customer->id,
                        'item_id' => $item->id,
                        'duration' => (int)$duration,
                        'total_amount' => $item->price * (int)$duration,
            ...
            $customer->update(['checkout_completed' => true]);

            // Send activation email (wrapped so failure doesn't rollback)
            try {
                Mail::raw(
                    "Hi {$customer->name},\n\nYour subscription is active. Thank you for choosing SubPilot.",
                    function ($message) use ($customer) {
                        $message->to($customer->email)->subject('SubPilot — Subscription Activated');
                    }
                );
            } catch (\Throwable $e) {
                Log::warning("Subscription activation mail failed for {$customer->email}: " . $e->getMessage());
            }
        });

        // Clear session
        session()->forget(['checkout_items', 'checkout_duration', 'checkout_product_id', 'test_otp', 'test_otp_mail_sent']);

        // Redirect to frontend home (your route name)
        return redirect()->route('home')->with('success', '✅ Subscription completed successfully!');
    }
}
